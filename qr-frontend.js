...
(function () {
  ...
  function init() {
    ...
    // 1. window.QR_FRONTEND_LANGUAGE (global override)
    // 2. Script tag data-language attribute
    // 3. Script URL ?language= parameter
    // 4. Container data-language attribute
    // 5. Default: 'de'

    var detectedLang = 'de'; // Default

    // Check container attribute (lowest priority)
    if (containerLang) detectedLang = containerLang;

    // Check script src URL parameter (higher priority)
    if (langFromCur) detectedLang = langFromCur;

    // Check for script tag in DOM (in case currentScript didn't work)
    ...
    // HIGHEST PRIORITY: Global override variable
    if (window.QR_FRONTEND_LANGUAGE) {
      var gl = String(window.QR_FRONTEND_LANGUAGE).trim().toLowerCase();
      if (gl) detectedLang = gl;
    }

    language = detectedLang;
    console.log('[QR-Frontend v1.6] Language:', language, 'Sources: container=' + containerLang + ', script=' + langFromCur + ', override=' + window.QR_FRONTEND_LANGUAGE);
    ...
    function runApp() {
      ...
      function showCustomerInfo(customer, totalPoints, codeId, bonusInfo, recentVisits) {
        ...
        var dateFields = ['date', 'validated_at', 'visited_at', 'visit_at', 'validation_date', 'created_at', 'timestamp', 'time'];
        var staffFields = ['staff', 'generated_by', 'generated_by_name', 'generatedBy', 'staff_name', 'staffName', 'employee', 'employee_name', 'created_by'];
        var parsed = (recentVisits || []).map(function (entry) {
          if (!entry) return null;
          var d = null;
          for (var i = 0; i < dateFields.length; i++) { var v = entry[dateFields[i]]; if (v) { d = v; break; } }
          if (!d) d = entry;
          var dateObj = new Date(d);
          if (isNaN(dateObj.getTime())) return null;
          var staff = null;
          for (var j = 0; j < staffFields.length; j++) { var s = entry[staffFields[j]]; if (s && String(s).trim()) { staff = String(s).trim(); break; } }
          return { date: dateObj, note: entry.note, staff: staff };
        ...
        visitHistoryList.innerHTML = history.length ? history.map(function (item) {
          ...
          var staffStr = item.staff ? '<span class="note">' + T.staff + ': ' + escapeHtml(item.staff) + '</span>' : '';
          var noteStr = item.note && String(item.note).trim() ? '<span class="note">' + escapeHtml(item.note) + '</span>' : '';
          var details = (staffStr + noteStr) || '<span class="note muted">—</span>';
          return '<li><span class="visit-date">' + dateStr + '</span><span class="visit-details">' + details + '</span></li>';
        }).join('') : '<li class="muted">' + T.noData + '</li>';
        if (history[0]) {
          var dateStr = formatter.format(history[0].date);
          var todayStart = new Date();
          todayStart.setHours(0, 0, 0, 0);
          var visitStart = new Date(history[0].date.getTime());
          visitStart.setHours(0, 0, 0, 0);
          var diffDays = Math.max(0, Math.floor((todayStart.getTime() - visitStart.getTime()) / 86400000));
          var daysAgoStr = diffDays === 0 ? T.today : T.daysAgo.replace('{n}', diffDays);
          lastVisitSummaryEl.textContent = dateStr + ' (' + daysAgoStr + ')';
        } else {
          lastVisitSummaryEl.textContent = '—';
        }
      }
