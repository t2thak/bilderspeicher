/**
 * QR-Generator Widget (eine Datei) – webhook + branch_id, keine Tenant nötig.
 * Ruft auf: {webhook}_qrcode, {webhook}_branches, {webhook}_qr_status
 *
 * Einbindung (z.B. Softr) – nur data-Attribute, kein Inline-Script:
 * <script src="https://cdn.jsdelivr.net/gh/t2thak/bilderspeicher@main/embed.js" data-webhook="justai" data-branch-id="DEINE-BRANCH-ID" data-webhook-base="https://n8nv2.flowrk.io/webhook/" data-language="de"></script>
 * <div id="qr-generator-container"></div>
 *
 * Alternative: window.QR_CONFIG = { webhook, branch_id, webhook_base?, language? };
 */
(function() {
  'use strict';
  ...
  var scriptEl = document.currentScript || (function() {
    ...
    for (var i = scripts.length - 1; i >= 0; i--) {
      var s = scripts[i];
      if (s.src && (s.src.indexOf("embed.js") !== -1 || s.src.indexOf("qr-widget") !== -1)) return s;
    }
    return null;
  })();
  if (scriptEl) {
    var getData = function(name) { var v = scriptEl.getAttribute("data-" + name); return (v && v.trim()) ? v.trim() : ""; };
    if (!CONFIG.webhook) CONFIG.webhook = getData("webhook");
    if (!CONFIG.branch_id) CONFIG.branch_id = getData("branch-id") || getData("branch_id");
    if (!CONFIG.webhook_base) CONFIG.webhook_base = getData("webhook-base") || getData("webhook_base");
    if (!CONFIG.language) CONFIG.language = getData("language");
  }
  try {
    var params = new URLSearchParams(window.location.search);
    if (params.get("webhook")) CONFIG.webhook = params.get("webhook");
    if (params.get("branch_id")) CONFIG.branch_id = params.get("branch_id");
    if (params.get("webhook_base")) CONFIG.webhook_base = params.get("webhook_base");
